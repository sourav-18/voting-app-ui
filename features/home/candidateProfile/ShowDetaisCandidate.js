...
import { removeNameAsync, selectMessFromCandidate, updteMessageFromCandidaet } from './candidateSlice';

const ShowDetaisCandidate = () => {
    const [buttonPress,setButtonPress]=useState(false)
    const navigation=useNavigation()
    const dispatch=useDispatch();
    const messageData=useSelector(selectMessFromCandidate)
    ...
    const candidateEndDate=data.periodOfTimeVoterRegistration.startTime;
    ...
    return (
        <View style={styles.container}>
           ...
           { today<candidateEndDate?<Button title='remove name form Election' onPress={()=>handleRemove(data._id)}/>:null}
