...
import { selectRegisterElection, voterShowRegisterElectionAsync } from '../voterHomeSlice';
import {styles} from "./VoterUpcomeingElection"
import { showElectionAsync, showResultAsync } from '../adminProfile/adminSlice';
import { useNavigation } from '@react-navigation/native';
export default function VoterRegisterElection() {
  const navigation=useNavigation()
  const dispatch=useDispatch();
  const data=useSelector(selectRegisterElection)
  useEffect(()=>{
    dispatch(voterShowRegisterElectionAsync());
  },[])
...
const handleShowDetails=(_id)=>{
  ...
  navigation.navigate("showElectionDetaisByVoter")
}
