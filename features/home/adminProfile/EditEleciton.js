import {
  ...
  import { createNewElectionAsync,selectAdminMessage,selectElection,selectcreateNewElectionData, updateElectionAsync, updateMessageDataFromAdmin, updatecreateNewElectionData } from "./adminSlice";
...
import Emty from "../../../Emty";
  const AddNewElection = () => {
    ...
    function numberToDate(timestamp) {
    ...
    //here error may be occure
    if(!ElectionData||!ElectionData.data){
        return (
            <Emty/>
        )
    }
    const Edata=ElectionData.data;
    const [candidateRegistrationDate, setCandidateRegistrationDate] = useState({
      stateDate: numberToDate(Edata.periodOfTimeCandidateRegistration.startTime),
      endDate: numberToDate(Edata.periodOfTimeCandidateRegistration.endTime),
    });
    const [voterRegistrationDate, setVoterRegistrationDate] = useState({
      stateDate: numberToDate(Edata.periodOfTimeVoterRegistration.startTime),
      endDate: numberToDate(Edata.periodOfTimeVoterRegistration.endTime),
    });
    const [votingDate, setVotingDate] = useState(numberToDate(Edata.dateOfVotiong));
    const [resultDate, setResultDate] = useState(numberToDate(Edata.resultDate));
    const [electionName, setElectionName] = useState(Edata.name);
    const [electionDesctiption, setElectionDesctiption] = useState(Edata.description);
    const [electionType, setElectionType] = useState(Edata.type);
    ...
    const handleUpdate = (_id) => {
      Alert.alert("Edit Election","Are you Sure To Edit This !",[
        {
          text: 'No',
        },
        {
            ...
            onPress:()=>{
              ...
              if (
                ...
                // console.log(data)
                dispatch(updateElectionAsync({updateData,_id}))
              } else {
                alert("fill all date");
              }
              }
    ...
    return (
      <View>
        <ScrollView>
          ...
          <Button onPress={() => handleUpdate(Edata._id)} title="Edit" />
          </View>
          {buttonPress&&messageData&&<Text style={innerStyle.message}>{messageData.message}</Text>}
        </ScrollView>
      </View>
    );
  };
